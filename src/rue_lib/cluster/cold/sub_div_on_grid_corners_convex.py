...
def make_convex_corner(
    vertex_point: Point, edge_vec0: np.ndarray, edge_vec1: np.ndarray, part_depth: float
) -> Polygon:
    """
...
def find_touching_edge(
    edges_gdf: gpd.GeoDataFrame, point: Point, tolerance: float = 0.1
) -> Optional[int]:
    """
...
def copy_attributes(source_row: gpd.GeoSeries, target_gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
...
def subdivide_part_at_convex_corners(
    ...
    # Find off-grid edges
    og_edges = part_edges_gdf[part_edges_gdf.get("road_type", "") == "off_grid"]
    ...
    for _, og_edge in og_edges.iterrows():
        ...
        for edge_coord in edge_coords:
            ...
            for i, poly_coord in enumerate(coords):
                if (
                    abs(poly_coord[0] - edge_coord[0]) < 0.01
                    and abs(poly_coord[1] - edge_coord[1]) < 0.01
                ):
                    ...
                    if angle > ang_threshold:
                        ...
                        if len0 > (plot_loc_w / 2) and len1 > (plot_loc_w / 2):
                            ...
                            corner = make_convex_corner(Point(p_curr), vec0_3d, vec1_3d, part_depth)
...
def process_part(
    ...
    strips, corners = subdivide_part_at_convex_corners(
        part_row, part_edges_gdf, part_sec_d, part_loc_d, plot_sec_w, plot_loc_w
    )
    ...
    for strip in strips:
        result = {"geometry": strip, "type": part_type, "class": "part"}
    ...
    for corner in corners:
        result = {"geometry": corner, "type": corner_type, "class": "part"}
        # Copy block attributes
        for attr in ["block_id", "block_type", "site"]:
            if attr in part_row.index:
                result[attr] = part_row[attr]
...
def subdivide_on_grid_corners_convex(
    ...
    for _idx, part_row in on_grid_parts.iterrows():
        subdivided = process_part(
            part_row, roads_layer, part_sec_d, part_loc_d, plot_sec_w, plot_loc_w
        )
