...
def subtract_cold_clusters_from_off_grid_blocks(
    input_gpkg: str,
    off_grid_blocks_layer_name: str,
    ...
    """
    Subtract off-grid cold clusters from off-grid blocks to create remaining loc_loc clusters.
    ...
    print("\nSubtracting cold clusters from off-grid blocks to create loc_loc clusters...")
    ...
    if gdf_cold.empty:
        print("  No cold clusters to subtract, using original blocks")
    ...
    for _, row in gdf_cold.iterrows():
        ...
        if orig_id is not None:
            ...
            cold_by_orig_id[orig_id].append(row.geometry)
    ...
    for _, block_row in gdf_blocks.iterrows():
        ...
        if block_id in cold_by_orig_id:
            cold_geoms = cold_by_orig_id[block_id]
            cold_union = unary_union(cold_geoms)

            try:
                remaining = block_geom.difference(cold_union)

                if remaining.is_empty or remaining.area <= 0:
                    print(f"    Block {block_id}: completely removed by cold clusters")
                    continue
